...
export class ReportingService implements OnInit {
  ...
  getreportingCommande(): any{
     this.http
      .get(environment.HTTP + '/reprting/commande').subscribe(data => {this.reportingCommande = data; }, error => {alert(error.message); });
  }
  getreportingFacture(dateDebut: any, dateFin: any): Observable<any>{
     return this.http
      .get(environment.HTTP + 'reprtings/factures/' + dateDebut + '/' + dateFin);
  }
  getreportingRetours(dateDebut: any, dateFin: any): Observable<any>{
     return this.http
      .get(environment.HTTP  + 'reprtings/retours/' + dateDebut + '/' + dateFin);
  }
