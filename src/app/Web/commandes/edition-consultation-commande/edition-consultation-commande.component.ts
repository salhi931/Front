...
import {DialogArticleConsulteComponent} from "../../articles/dialog-article-consulte/dialog-article-consulte.component";
...
export class EditionConsultationCommandeComponent implements OnInit {
  ...
  constructor(public gestionDesCommandesService: GestionDesCommandes2Service, public  gestionDesArticlesService: GestionDesArticlesService,
              public dialog: MatDialog, public configurationService: ConfigurationService, private router: Router, private route: ActivatedRoute) { }

  ngOnInit(): void {
    console.log(this.gestionDesCommandesService.articlescommandees);
    this.gestionDesCommandesService.rechercherClient(this.gestionDesCommandesService.commande.id_client)
      .subscribe(data => {
        this.client = data;
        console.log(this.gestionDesCommandesService.commande.id_client);

      });
    // tslint:disable-next-line:max-line-length
    if (!this.gestionDesCommandesService.commande || this.gestionDesCommandesService.commande === undefined || this.gestionDesCommandesService.commande.length === 0 ){
  ...
  openDialog(): void{
    this.dialog.open(DialogImagesComponent, {data: this.gestionDesArticlesService.selectedFileListConsulte, height: '700px',
      width: '700px'});
