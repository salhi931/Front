...
export class StockComponent implements OnInit {
  ...
  filtrage: any;
  displayedColumns2: string[] = ['N.o', 'article', 'quantite', 'quantiteMobile',  'edit'];
  // @ts-ignore
  ...
  openDialog2(stock: any): void{
    ...
    dialogref.afterClosed().subscribe(data => {
      if (data && data !== ''){
         this.gestionDeStockService.modifierStock(data, stock)
          .subscribe(data1 => {
            stock.quantite = data;
  ...
  MiseAjourStock(idMagasin: any): any{
    this.gestionDeStockService.UpdateMagasinStock(idMagasin)
      .subscribe(data => {
        if (data === 0){
          alert('stock a été mise a jour avec succés');
          this.onModifier(idMagasin);
        }
        if (data === -1){
          alert('une error a été détéctée');
        }
        if (data > 0){
          alert ( 'Vous avez encore des factures-retours nons pas encore synchronisées (' + data + ')');
        }
      }, error => {alert(error.message); });
  }
}
