...
export class GestionDesFacturesService implements OnInit{
  ...
  getPaiementLiee(numfacture: any): Observable<any> {
    return this.http
      .get(environment.HTTP + 'getPaiementLiee/' + numfacture);
  }
  getCommericaux(): void{
    this.http.
      ...
      .subscribe(datax => {
        ...
        this.getMagasinsList()
          .subscribe(dataxx => {
            ...
            this.http
              .get(environment.HTTP + 'getFactureWeb/' + id)
                  .subscribe(data => {
                    // @ts-ignore
                    ...
                    for (const facturedetails of facture_detail){
                      //alert(facture.id_article);
                      facturedetails.code_article = this.getCodeArticle(facturedetails.id_article);
                      facturedetails.description = this.getDescriptionArticle(facturedetails.id_article);
                      facturedetails.magasin = this.getMagasinNom(facturedetails.id_magasin);
                    }
                    // @ts-ignore
                    facture.code = this.clients.filter(client => client.id_client === facture.idClient);
                    // @ts-ignore
                    facture.nomCommercial = this.commerciaux.filter(commercial => commercial.id_commercial === facture.idCommercial);
  ...
  getFactureWeb(id: any): Observable<any>{
    return this.http
      .get(environment.HTTP + 'getFactureWeb/' + id);
  }
  getFacturesCommercialDate(f: any): Observable<any>{
    if (f.value.date !== ''){
      return this.http
        .get(environment.HTTP + 'getFacturesCommercialDate/' + f.value.idCommercial + '/' + formatDate(f.value.date, 'yyyy-MM-dd', 'en-US') );
        //.get(environment.HTTP + 'getFacturesCommercialDate/' + f.value.idCommercial + '/' + f.value.date );
    }
    else{
      return this.http
          .get(environment.HTTP + 'getFacturesCommercial/' + f.value.idCommercial);
    }
  }
  getMagasins(): any{
    this.magasins = [];
  ...
  getNomCommercialPaiement(id: any): any{
    for (const commercial of this.commerciaux){
  ...
  getImage(id: number): Observable<any>{
    ...
    return this.http.get(environment.HTTP + 'imageFactures/get/' + id);
  }
  rechercheClientPaiement(id: any): any{
    // @ts-ignore
    const client = this.clients.filter(client => client.idClient === id);
  }
