...
export class GestionDesRetoursService implements OnInit{
                ...
                .subscribe(data => {
                  ...
                  this.http
                    ...
                    .subscribe(data1 => {
                      ...
                      for (const retour of this.retours_details){
                        retour.code_article = this.getCodeArticle(retour.id_article);
                        retour.description = this.getDescriptionArticle(retour.id_article);
                        retour.magasin = this.getMagasinNom(retour.id_magasin);
                      }
  ...
  changestatusretours(id: any, retour: any): Observable<any>{
     return this.http
      .patch(environment.HTTP + 'retourses/' + id, retour);
  }
  ValiderTousLesretours(list: any, Data1: any): Observable<any>{
    console.log(Data1);
    return this.http.post(environment.HTTP + 'validerLesRetours8', list);
  }
  ValiderTousLesretoursDetails(list: any, Data1: any): Observable<any>{
    return this.http.post(environment.HTTP + 'validerLesRetoursDetails', {retour: list, retourdetail: Data1});
  }
  getRetoursCommercial(list: any): Observable<any>{
    return this.http.post(environment.HTTP + 'getRetoursCommercial' , list);
  }
  rechercheElement(L: any, element: any): number{
    let i = 0;
    for (const elmt of L){
      if (elmt.codeArticle === element.codeArticle && elmt.motif === element.motif){
        return i;
      }
      i += 1;
    }
