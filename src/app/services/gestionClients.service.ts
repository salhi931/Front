...
export class GestionClientsService implements OnInit{
 ...
 // client_a_ajoutee: Client = new Client();
  ...
  getFeedbackbyClient(id: any): Observable<any> {
    return this.http
      .get(environment.HTTP + 'getFeedbackbyClient /' + id);
  }
  getFeedback(): Observable<any> {
    return this.http
      .get(environment.HTTP + 'getFeedbackAll');
  }
  ...
  getCategorieClient(): any{
    this.http
      .get(environment.HTTP + '/getClientsCategories')
      .subscribe(data => {this.CategorieClient = data; }, error => {alert(error.message); });
  }
  getCategorieClientReturn(): Observable<any>{
    return this.http
      .get(environment.HTTP + '/getClientsCategories');
  }
      ...
      .subscribe( data1 => {
            ...
            this.http
              ...
              .subscribe(data => {
                ...
                localStorage.setItem('idClient', String(id));
                this.router.navigate(['/Edition-consultation-client'], {relativeTo: this.route});
              });
  });
  }
  getClient_consultee2(id: number): void{
    // this.client_a_modifier.emit(id);
    this.getTableprix();
    this.getCategorieClient();
    this.getzoness()
  ...
  getClientBigdata(): Observable<any>{
    return this.http
      .get(environment.HTTP + 'clientses');
  }
  ClientsByCode(code: any): Observable<any>{
    return this.http
      .get(environment.HTTP + 'ClientsByCode/' + code);
  }

  ClientsByNom(code: any): Observable<any>{
    return this.http
      .get(environment.HTTP + 'ClientsByNom/' + code);
  }
  getPiece(id: any): Observable<any>{
    return this.http
      .get(environment.HTTP + 'imageFeedback/get/' + id);
  }
  ...
  //******************************************************************************feedbacks *****************************************************************************
  getFeedbacksCommercialDate(f: any): Observable<any>{
    if (f.value.date !== ''){
      return this.http
        .get(environment.HTTP + 'getFeedbacksCommercialDate/' + f.value.idCommercial + '/' + formatDate(f.value.date, 'yyyy-MM-dd', 'en-US') );
      //.get(environment.HTTP + 'getFacturesCommercialDate/' + f.value.idCommercial + '/' + f.value.date );
    }
    else{
      return this.http
        .get(environment.HTTP + 'getFeedbacksCommercial/' + f.value.idCommercial);
    }
  }
}
