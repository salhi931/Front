...
export class GestionDesCommandesService implements OnInit{
  ...
  getCommandeWeb(id: any): Observable<any>{
    return this.http
  ...
  changestatus(id: any, status: boolean): Observable<any>{
    return this.http
      .get(environment.HTTP + 'commandes/' + id, );
  }
  getCommandesCommercialDate(f: any): Observable<any>{
    if (f.value.date !== ''){
      return this.http
        .get(environment.HTTP + 'getCommandesCommercialDate/' + f.value.idCommercial + '/' + formatDate(f.value.date, 'yyyy-MM-dd', 'en-US') );
      //.get(environment.HTTP + 'getFacturesCommercialDate/' + f.value.idCommercial + '/' + f.value.date );
    }
    else{
      return this.http
        .get(environment.HTTP + 'getCommandesCommercial/' + f.value.idCommercial);
    }
  }
  getImage(id: number): Observable<any>{
    // Make a call to Sprinf Boot to get the Image Bytes.
    return this.http.get(environment.HTTP + 'imageCommandes/get/' + id);
  }
}
